...
class AcceptsSpecJetty extends Specification with unfiltered.specs2.jetty.Planned with AcceptsSpec

class AcceptsSpecNetty extends Specification with unfiltered.specs2.netty.Planned with AcceptsSpec
...
trait AcceptsSpec extends Specification with unfiltered.specs2.Hosted {
  ...
  "Accepts should" should {
    ...
    "match a mixed accepts request with json as json" in {
      val resp = http(
        req(host / "test") <:< Map(
          "Accept" -> "application/xhtml+xml,text/xml;q=0.5,application/xml;q=0.9,*/*;q=0.8,application/json"
        )
      ).as_string
      resp must_== "json"
    }
    ...
    "match a mixed accepts request with xml as xml" in {
      val resp = http(
        req(host / "test") <:< Map(
          "Accept" -> "text/html,application/xhtml+xml,text/xml;q=0.5,application/xml;q=0.9,*/*;q=0.8"
        )
      ).as_string
      resp must_== "xml"
    }
    ...
    "match a mixed accepts request with html as html" in {
      val resp = http(
        req(host / "test") <:< Map("Accept" -> "application/octet-stream,text/html,application/pdf;q=0.9,*/*;q=0.8")
      ).as_string
      resp must_== "html"
    }
    ...
    "match a mixed accepts request with csv as csv" in {
      val resp =
        http(req(host / "test") <:< Map("Accept" -> "text/html,text/csv,application/pdf;q=0.9,*/*;q=0.8")).as_string
      resp must_== "csv"
    }
