  ...
  "FromCookies" should {
    "parse a version 0 cookie" in {
      val cs = "myCookie=myValue;expires=XXX;path=/apathsomewhere;domain=.adomainsomewhere;secure;".replace(
        "XXX",
        DateFormatting.format(new Date(System.currentTimeMillis() + 50000))
      )
    ...
    "parse a single version 0 cookie ignoring extra params" in {
      val cs =
        "myCookie=myValue;max-age=50;path=/apathsomewhere;domain=.adomainsomewhere;secure;comment=this is a comment;version=0;commentURL=http://aurl.com;port=\"80,8080\";discard;"
      val cookies = FromCookies(cs)
      cookies.size must_== 1
      val cookie = cookies(0)
      cookie.value must_== "myValue"
    ...
    "parse a simple version 0 cookie" in {
      val cs =
        "myCookie=myValue;max-age=50;path=/apathsomewhere;domain=.adomainsomewhere;secure;comment=this is a comment;version=1;"
      val cookies = FromCookies(cs)
      cookies.size must_== 1
      val cookie = cookies(0)
      cookie.value must_== "myValue"
    ...
    "parse a single version 1 cookie w/ extra params ignored" in {
      val cookieString =
        "myCookie=myValue;max-age=50;path=/apathsomewhere;domain=.adomainsomewhere;secure;comment=this is a comment;version=1;commentURL=http://aurl.com;port='80,8080';discard;"
