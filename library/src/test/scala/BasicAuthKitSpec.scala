...
class BasicAuthKitSpecJetty extends Specification with unfiltered.specs2.jetty.Planned with BasicAuthKitSpec

class BasicAuthKitSpecNetty extends Specification with unfiltered.specs2.netty.Planned with BasicAuthKitSpec

trait BasicAuthKitSpec extends Specification with unfiltered.specs2.Hosted {
  import unfiltered.response._

  def valid(u: String, p: String) = (u, p) match {
    case ("test", "secret") => true
    case _ => false
  }

  def intent[A, B]: unfiltered.Cycle.Intent[A, B] = unfiltered.kit.Auth.basic(valid) { case _ =>
    ResponseString("we're in")
  }
