...
class JsonpSpec extends Specification with unfiltered.specs2.jetty.Served {
  ...
  class TestPlan extends unfiltered.filter.Plan {
    def intent = {
      ...
      case GET(UFPath("/jsonp/lift-json") & Jsonp(callback)) =>
        callback respond {
          import org.json4s._
          JArray(JInt(42) :: Nil)
        }
  ...
  "Jsonp should" should {
    "match an text/javascript accepts request with callback, wrapping response body in callback" in {
      val resp =
        http(req(host / "jsonp" <<? Map("callback" -> "onResp")) <:< Map("Accept" -> "text/javascript")).as_string
    ...
    "optionally match an text/javascript accepts request with callback, wrapping response body in callback" in {
      val resp = http(
        req(host / "jsonp" / "optional" <<? Map("callback" -> "onResp")) <:< Map("Accept" -> "text/javascript")
      ).as_string
      resp must_== "onResp([42])"
    }
    ...
    "produce a jsonp response, wrapping response body in callback" in {
      val resp = http(
        req(host / "jsonp" / "lift-json" <<? Map("callback" -> "onResp"))
    ...
    "optionally produce a json response when callback is missing" in {
      val resp = http(
        req(host / "jsonp" / "lift-json" / "optional")
