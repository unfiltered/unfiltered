...
class MixedPlanSpec extends Specification with Served {
  ...
  def setup = {
    _.plan(cycle.MultiPartDecoder {
      ...
      case POST(UFPath("/cycle/pass")) =>
        cycle.MultipartPlan.Pass
      case POST(UFPath("/cycle/disk") & MultiPart(req)) => { case Decode(binding) =>
      ...
      case POST(UFPath("/cycle/stream") & MultiPart(req)) => { case Decode(binding) =>
      ...
      case POST(UFPath("/cycle/mem") & MultiPart(req)) => { case Decode(binding) =>
      ...
      case POST(UFPath("/cycle/pass")) & MultiPart(req) => { case Decode(binding) =>
        val disk = MultiPartParams.Disk(binding)
        (disk.files("f"), disk.params("p")) match {
          case (Seq(f, _*), p) =>
            ResponseString(
      ...
      case POST(UFPath("/async/pass")) =>
        async.MultipartPlan.Pass
      case POST(UFPath("/async/disk") & MultiPart(req)) => { case Decode(binding) =>
        val disk = MultiPartParams.Disk(binding)
        (disk.files("f"), disk.params("p")) match {
          case (Seq(f, _*), p) =>
            binding.respond(
      ...
      case POST(UFPath("/async/stream") & MultiPart(req)) => { case Decode(binding) =>
        val stream = MultiPartParams.Streamed(binding)
        (stream.files("f"), stream.params("p")) match {
          case (Seq(f, _*), p) =>
            binding.respond(
      ...
      case POST(UFPath("/async/mem") & MultiPart(req)) => { case Decode(binding) =>
        val mem = MultiPartParams.Memory(binding)
        (mem.files("f"), mem.params("p")) match {
          case (Seq(f, _*), p) =>
            binding.respond(
      ...
      case POST(UFPath("/async/pass") & MultiPart(req)) => { case Decode(binding) =>
        val disk = MultiPartParams.Disk(binding)
        (disk.files("f"), disk.params("p")) match {
          case (Seq(f, _*), p) =>
            binding.respond(
  ...
  "Netty MultiPartDecoder cycle and async plans, when used in the same pipeline" should {
    ...
    "handle cycle file uploads disk" in {
      ...
      file.exists must_== true
      http(
        req(host / "cycle" / "disk").<<*("f", file, "text/plain")
      ).as_string must_== "cycle disk read file f named netty-upload-big-text-test.txt with content type text/plain and param p List()"
    }
    ...
    "handle cycle file uploads streamed" in {
      ...
      file.exists must_== true
      http(
        req(host / "cycle" / "stream").<<*("f", file, "text/plain")
      ).as_string must_== "cycle stream read file f is named netty-upload-big-text-test.txt with content type text/plain and param p List()"
    }
    ...
    "handle cycle file uploads memory" in {
      ...
      file.exists must_== true
      http(
        req(host / "cycle" / "mem").<<*("f", file, "text/plain")
      ).as_string must_== "cycle memory read file f is named netty-upload-big-text-test.txt with content type text/plain and param p List()"
    }
    ...
    "handle passed cycle file uploads to disk" in {
      ...
      file.exists must_== true
      http(
        req(host / "cycle" / "pass").<<*("f", file, "text/plain")
      ).as_string must_== "cycle disk read file f named netty-upload-big-text-test.txt with content type text/plain and param p List()"
    }
    ...
    "handle async file uploads to disk" in {
      ...
      file.exists must_== true
      http(
        req(host / "async" / "disk").<<*("f", file, "text/plain")
      ).as_string must_== "async disk read file f named netty-upload-big-text-test.txt with content type text/plain and param List()"
    }
    ...
    "handle async file uploads streamed" in {
      ...
      file.exists must_== true
      http(
        req(host / "async" / "stream").<<*("f", file, "text/plain")
      ).as_string must_== "async stream read file f is named netty-upload-big-text-test.txt with content type text/plain and param p List()"
    }
    ...
    "handle async file uploads memory" in {
      ...
      file.exists must_== true
      http(
        req(host / "async" / "mem").<<*("f", file, "text/plain")
      ).as_string must_== "async memory read file f is named netty-upload-big-text-test.txt with content type text/plain and param p List()"
    }
    ...
    "handle passed async file uploads to disk" in {
      ...
      file.exists must_== true
      http(
        req(host / "async" / "pass").<<*("f", file, "text/plain")
      ).as_string must_== "async disk read file f named netty-upload-big-text-test.txt with content type text/plain and param List()"
    }
