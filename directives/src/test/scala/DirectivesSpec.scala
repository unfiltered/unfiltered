...
object DirectivesSpecJetty extends Specification with unfiltered.specs2.jetty.Planned with DirectivesSpec

object DirectivesSpecNetty extends Specification with unfiltered.specs2.netty.Planned with DirectivesSpec
...
trait DirectivesSpec extends SpecificationLike with unfiltered.specs2.Hosted {
  ...
  case class BadParam(msg: String)
      extends ResponseJoiner(msg)(messages => BadRequest ~> ResponseString(messages.mkString("\n")))
  ...
  val asEven = data.Conditional[Int](_ % 2 == 0).fail((name, i) => BadParam(name + " is not even: " + i))
  ...
  "Directives" should {
    ...
    "respond with expected response given named value" in {
      def expect(n: Int) = {
        ...
        val expected =
          if (n < MaxPrizes) "Congratulations. You won prize %d".format(n + 1) else "prizes are out of stock"
        resp must_== expected
      }
  ...
  "Directive independent parameters" should {
    "respond with parameter if accepted" in {
      val resp = http(
        req(host / "independent_parameters")
          << Map(
            "option_int" -> 3.toString,
            "require_int" -> 4.toString,
            "even_int" -> 8.toString
    ...
    "respond with all errors" in {
      ...
      resp.code must_== 400
      resp.as_string must_== Seq("option_int is not an int: four", "require_int is missing", "even_int is not even: 7")
        .mkString("\n")
    }
  }
}
