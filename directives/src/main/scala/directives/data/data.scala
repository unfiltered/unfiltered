...
trait Interpreter[A, B, +E] { self =>
  ...
  def ~>[C, EE >: E](implicit next: Interpreter[B, C, EE]): Interpreter[A, C, EE] =
    (a: A, name: String) =>
      self.interpret(a, name).flatMap { r =>
        next.interpret(r, name)
      }
...
class Strict[A, B, +E](cf: A => Option[B], handle: (String, A) => E) extends Interpreter[Option[A], Option[B], E] {
...
class Requiring[A, +E](handle: String => E) extends Interpreter[Option[A], A, E] {
